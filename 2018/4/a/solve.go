...
func main() {
  ...
  for _, g := range sched {
    ...
    if g.totalSleep > sleepy.totalSleep {
      sleepy = g
    }
  }

  log.Printf("Sleepiest guard: %d (%d)", sleepy.id, sleepy.totalSleep)
  log.Printf("Days on duty: %d", len(sleepy.log))
  log.Printf("Sleep schedule: %v", sleepy.schedule)
  var most, best int
  for min, times := range sleepy.schedule {
    if times > most {
      best = min
      most = times
    }
  ...
  log.Printf("Best time: %d (%d sleeps)", best, most)
  log.Printf("Answer: %d", best * sleepy.id)
}
